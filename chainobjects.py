...
bl_info = {
    ...
    "author": "Michel Anders (varkenvarken), Updated to Blender 2.80 by Satish Goda (iluvblender)",
    "version": (0, 1, 0),
    "blender": (2, 80, 0),
...
class ChainSelectedObjects(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        while objects:
            ...
            child.select_set(False, view_layer=view_layer)
            ob = child
        first.select_set(True, view_layer=view_layer)
...
def register():
    bpy.utils.register_class(ChainSelectedObjects)
    bpy.types.VIEW3D_MT_object.append(menu_func)


def unregister():
    bpy.types.VIEW3D_MT_object.remove(menu_func)
    bpy.utils.unregister_class(ChainSelectedObjects)
